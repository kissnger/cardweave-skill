...
curate.py           ──→  读 _meta.date 筛选 DB 条目
                         ↓
generate.py -o ../output  →  ../output/{_meta.date}/  (输出日期子目录)
```
...
18. **输出放 skill 目录外：** 所有产物用 `generate.py -o ../output` 输出到 `../output/{date}/`。技能目录内置硬门禁：`-o` 路径在技能树内直接拒绝。
...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-05-26 | 5.0.1 | 安全门禁：generate.py 硬检测 -o 路径，落在技能树内直接拒绝；所有文档输出示例改为 ../output |
| 2026-05-26 | 5.0.0 | 重构：技能名 cardweave → cardweave-skill，目录结构模块化（pipeline/lib/config/assets/templates/references），所有路径改为相对引用 |
