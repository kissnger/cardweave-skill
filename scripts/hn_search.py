#!/usr/bin/env python3
"""
Hacker News 搜索工具（Algolia API）

比官方 Firebase API 快得多，支持关键词搜索、按分数筛选、按时间范围。
无认证，免费使用。

用法：
  ./hn_search.py                                         # Show HN 最新 10 条
  ./hn_search.py --top                                   # 首页热门前 10
  ./hn_search.py --search "AI agent"                     # 按关键词搜索
  ./hn_search.py --search "code" --min 50                # 搜索 + 最低分数
  ./hn_search.py --days 7                                # 最近 7 天
  ./hn_search.py --popular --min 100                     # 历史高分
  ./hn_search.py --json                                  # 输出完整 JSON
  ./hn_search.py --cardweave                             # 输出 cardweave 格式
...
def search(params, sort_by_date=False):
    ...
    try:
        req = Request(url, headers={"User-Agent": "hn-search/1.0"})
...
def fetch(tags="show_hn", query="", min_points=0, days=0, count=10, sort_by_date=True):
    ...
    if filters:
        params.append("numericFilters=" + quote(",".join(filters)))
